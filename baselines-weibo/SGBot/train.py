...
def parse_args():
    parser = argparse.ArgumentParser(description="Train SGBot on Weibo")
...
def main():
    ...
    if len(agent_features) > 0:
        agent_pred = classifier.predict(agent_features)
        agent_prob = classifier.predict_proba(agent_features)[:, 1]
        agent_truth = np.ones(len(agent_features), dtype=np.int64)
        agent_metrics = compute_metrics(agent_truth, agent_pred, agent_prob)
        print_metrics("\nOOD (agent accounts, ground truth=bot)", agent_metrics)
    else:
        agent_metrics = None
        print("\nNo agent accounts found, skipping OOD evaluation.")

    checkpoint_path = CKPT_DIR / f"rf_seed{args.random_seed}.joblib"
    joblib.dump(
        {
            "model": classifier,
