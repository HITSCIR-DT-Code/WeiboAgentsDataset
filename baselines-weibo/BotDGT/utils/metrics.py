...
def compute_metrics_one_snapshot(y_true, y_output, exist_nodes):
    ...
    if torch.any(torch.isnan(y_output)):
        ...
        metrics['f1'] = round(f1_score(y_true, y_pred_label, average="macro"), 5)
        metrics['precision'] = round(precision_score(y_true, y_pred_label, average="macro"), 5)
        metrics['recall'] = round(recall_score(y_true, y_pred_label, average="macro"), 5)
    return metrics
